...
from vds.models import Project
...
class ProjectCreateTransmittalTests(TestCase):
    ...
    def test_init_first_transmittal_defaults(self):
        p = self._create_project()
        t_init = p.init_transmittal(source=None)
        self.assertEqual(t_init["project"], p.id)
        self.assertEqual(t_init["number"], 'TR-001')
        self.assertEqual(t_init["source"], 'HOUSE')
        self.assertEqual(t_init["date_sent"], datetime.date.today())
    ...
    def test_increment_trailing_digits_preserves_padding(self):
        ...
        t2 = p.init_transmittal(source='S1')
    ...
    def test_append_one_when_no_digits(self):
        ...
        t2 = p.init_transmittal(source='S2')
